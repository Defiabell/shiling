 ...
 *
 * **[S2] 250 → 500**：探索池从 20 条涨到 39 条，而深处那三处（幽潭／秘窟／焦原）要凑齐
 * 一两件特定器官才进得去 —— 一个不带血脉、随机蜕形的机器玩家一世里只有 7〜20% 的概率
 * 开得出它们（实测）。那不是「内容够不到」，是**采样不够**：同样的内容，250 世里每次
 * 漏掉的都是不同的一条（先是幽潭的「渊心」，再是「沉物」，再是「落鳞」）。
 * 500 世稳定 69/70，跑一秒出头。真正的可玩性数据看
 * `pnpm -C packages/gen balance --profile wayseek`（那个画像会主动往深处走）。
 */
const LIFE_COUNT = 500;
/**
