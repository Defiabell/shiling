...
export const BASELINE_TUNING: TaleTuning = {
   ...
   * 0.7³ ≈ 0.34，且**第一层就再也逃不掉**；三层眼伤 ＝ 它近八成打空，两层起不再反击。
   *
   * ## 这三个数被实验台调过三轮，每一轮都有一条被推翻的判断
   * 1. **第一版腿伤开太大**（出伤 0.86／扑权重 0.65）：把玄蟒（M1-P2 时 bare build 只有 20%
   *    胜率的那堵墙）推到 97.3% —— 「先堆满六层伤再收官」成了压倒性通解。收到 0.9／0.7。
   * 2. **第二版眼伤开太小**（0.12）：一层眼伤只买一成二的打空，而花掉的那一合本可以咬一口
   *    （1.6 vs 0.35 倍）—— 于是「它宣告重击 → 先弄瞎它」这一手变成负期望，**读得出意图的
   *    build 反而比读不出的更危险**（1200 场实测：死亡率 13.7% vs 13.6%，胜率还低 14 个点）。
   *    这正是 M1-P2 那条「信息被误用了」的重演，只是这一次错在数值不在链条。
   * 3. **收在 0.26**：一层眼伤在一场 8〜10 合的架里省下的伤害刚好抵得过那一合的输出，于是
   *    「什么时候该弄瞎它」重新成为一道题。实验台（800 场，对玄蟒）：裸 build 死亡率 13.4%、
   *    灵犀 6.5%（低一半），而玄蟒对 screen 打法仍只有 76.5% 胜率 —— 它还是那堵墙。
   ...
   *
   *   分量 ＝ 总系数×`forgePowerPerMul` ＋（断伤？`forgePowerWound`：0）＋ `forgeEffectPower[那一档]`
   *   精气 ＝ 分量（**不缩放、不四舍五入**）
   *   势   ＝ clamp(round(分量 / 7), 1, 3)　　冷却 ＝ clamp(1 + round(分量 / 9), 1, 4)
   *
   * 拿 owner 那张招式框原样对一遍：起手 齿（×1.4）＋ 力道 鬃（+0.4）＋ 附加 毒（venom 9）
   *   分量 ＝ 1.8×5 ＋ 0 ＋ 9 ＝ **18** → 精气 18 · 势 3 · 冷却 3
   * —— 与原型图上「代价：猛之精气 18　势 3 · 冷却 3」逐项相同。
   *
   * ## 为什么精气价**就是**分量（这一条是被占优闸门逼出来的）
   * 第一版写的是 `精气 ＝ round(分量 × 0.6)`，闸门当场报了 **832 对**严格占优 ——
   * 全是**取整撞车**：分量 23 与 24 都算成 14，于是「同价而伤害更高」的那一手让另一手
   * 从此没人会拼。任何缩放系数都会把某两档不同的分量压到同一个价钱上，而那正是
   * 「一条严格占优的拼法」的定义。所以缩放系数被删掉了：**价钱与分量一一对应**，
   * 「更强必然更贵」于是从「设计意图」升成了**结构性事实**。
   * 同一条理由让古法用**加价**（`forgeLoreSurcharge`）而不是乘一个 1.15。
   *
   * ## 三个上界
   * - 势上限 3：`encounterFinisherMomentum` 是 4 —— 一手招的势价必须低于决杀，
   *   否则决杀这颗按钮在有招的 build 上永远轮不到（它还要吃掉全部的势）。
   * - 冷却上限 4：`combatSkillCooldown` 缺省 3、古法到 6。凝成的招是「自己的常用手」，
   *   压在 4 以内才轮转得起来；真正长冷却的重手留给古法。
   * - 精气区间因此落在 **10〜25**（最省的一手 ×0.8 ＋ 硬受，最贵的一手 ×2.0 ＋ 断眼 ＋ 脱身）。
   *   `moltThreshold` 是 95，所以凝满四手 ≈ **少蜕一件器官** —— 槽位上限之外的第二道取舍。
   *
   * ## 十档效果的相对价（`forgeEffectPower`）
   * 判据统一为「它在一场 5〜10 合的架里折合多少点伤害」——同 S1 定三条持续类总量的办法：
   * 必定脱身把整场归零（最贵 11）＞ 顿挫直接偷一个回合（10）＞ 流血 3 合 ×2、迟滞削它的
   * 出手、蒙目让它多半打空、疗愈回 8 血（各 9）＞ 护体（8）＞ 明识、反刺（各 7，
   * 都要它肯出手才兑现）＞ 硬受只挡当下这一下（最便宜 6）。
   */
  forgeSlots: 4,
  forgePowerPerMul: 5,
  forgePowerWound: 4,
  ...
  forgeMomentumPerPower: 7,
  forgeMomentumMax: 3,
  forgeCooldownPerPower: 9,
  forgeCooldownMax: 4,
  forgeLoreSurcharge: 4,
  forgeNameMaxChars: 6,
