 ...
 *
 * 为什么要合：引擎对「探索但没抽到事件」只有一句固定旁白（[S2] 起带去处名，「往兽径去。」），
 * 连着往同一处探三季就在 6 条可见位里占掉三格一模一样的字，读起来像界面坏了 —— 而且把真正
 * 发生过的事挤出了栏外。合成「×3」既省位子，又如实表达「这几季什么都没发生」。
 * [S2] 换了去处就是另一句，于是**不再合并** —— 那正是想要的：「往幽潭去」与「往兽径去」
 * 是两件不同的事，日志上就该分开记。
